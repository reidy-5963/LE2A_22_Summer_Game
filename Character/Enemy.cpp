...
void Enemy::Initialize(const std::vector<Model*>& models, const Vector3 position, bool isLast) {
	...
	fireSEHAndle_ = Audio::GetInstance()->LoadWave("swish1_1.wav");
	damageSEHandle_ = Audio::GetInstance()->LoadWave("damageSE.wav");
...
void Enemy::Attack() {
	...
	if (isAttack_) {
		...
		Audio::GetInstance()->PlayWave(fireSEHAndle_, false, 0.05f);
