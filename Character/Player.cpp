...
void Player::Initialize(const std::vector<Model*>& models) {
	...
	fire_SEHandle_ = Audio::GetInstance()->LoadWave("swish1_1.wav");
	damageSEHandle_ = Audio::GetInstance()->LoadWave("PlayerDamageSE.wav");
	...
	// 体力表示の初期化
	hp_bar[0].reset(Sprite::Create(hp_tex_[0], hp_pos_));
	hp_gauge_[0].reset(Sprite::Create(hp_gauge_tex_[0], hp_gauge_pos_));
	hp_bar[1].reset(Sprite::Create(hp_tex_[1], hp_pos_));
	hp_gauge_[1].reset(Sprite::Create(hp_gauge_tex_[1], hp_gauge_pos_));
	hp_size_ = hp_bar[0]->GetSize();
...
void Player::Update() {
	...
	damageEffectAlpha_ = (1.0f - hp_ / setHp_);
	...
	//
	if (!isDenger_) {
		hp_bar[0]->SetPosition(hp_pos_);
		hp_bar[0]->SetSize({(hp_ / setHp_) * hp_size_.x, hp_size_.y});
		hp_gauge_[0]->SetPosition(hp_gauge_pos_);
	}
	//
	else if (isDenger_) {
		hp_bar[1]->SetPosition(hp_pos_);
		hp_bar[1]->SetSize({(hp_ / setHp_) * hp_size_.x, hp_size_.y});
		hp_gauge_[1]->SetPosition(hp_gauge_pos_);
	}
...
void Player::OnCollision(float damage) {
	if (!isDamage_) {
		Audio::GetInstance()->PlayWave(damageSEHandle_, false, 0.5f);
...
void Player::ApplyGlobalVariavles() {
	...
	hp_pos_ = gloVars->GetVector2Value(groupName, "hp_pos_");
	hp_gauge_pos_ = gloVars->GetVector2Value(groupName, "hp_gauge_pos_");
}
...
void Player::InitializeGlobalVariavles() {
	...
	gloVars->AddItem(groupName, "hp_pos_", hp_pos_);
	gloVars->AddItem(groupName, "hp_gauge_pos_", hp_gauge_pos_);
}
...
void Player::Attack() {
	...
	if (isAttack_) {
		...
		Audio::GetInstance()->PlayWave(fire_SEHandle_, false, 0.5f);
...
void Player::SetHeart(
    uint32_t hp_root,
	uint32_t hp_gauge_root,
	uint32_t hp_out,
	uint32_t hp_gauge_out) {
	hp_tex_[0] = hp_root;
	hp_gauge_tex_[0] = hp_gauge_root;
	hp_tex_[1] = hp_out;
	hp_gauge_tex_[1] = hp_gauge_out;
}
