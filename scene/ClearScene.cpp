...
void ClearScene::Initialize() {
	...
	BGMHandle_ = audio_->LoadWave("clear3.wav");
	selectSEHandle_ = audio_->LoadWave("select01.wav");

	isClearGimmick_ = false;
	clearGimmickTimer_ = 0.0f;

#pragma region ロゴ
	// タイトルロゴテクスチャの読み込み
	logoTex_ = TextureManager::Load("Game_Clear_Tex.png");
...
void ClearScene::KeyMouseUpdate() { 
	// 現在のカーソル位置を取得
...
void ClearScene::ApplyGlobalVariavles() { // グローバル変数系のシングルトンインスタンスを取得
	GlobalVariables* gloVars = GlobalVariables::GetInstance();
	// グループ名の設定
	const char* groupName = "CLEAR";
...
void ClearScene::JoyStateUpdate() {
	// コントローラー
	XINPUT_STATE joystate;
	if (input_->GetJoystickState(0, joystate)) {
		// もしフラグが有効なら
		...
		JoyState_ButtonUpdate(
		    END, backTitle_.get(), endButton_.get(), backTitleTex_[0], endButtonTex_[1],
		    isEnd_);
...
bool ClearScene::ButtonToCurSor(Vector2 buttonPos, Vector2 buttonSize) {
	POINT cursorPos;
	// 現在のカーソル位置を取得
...
void ClearScene::KeyMouse_ButtonUpdate(
...
void ClearScene::JoyState_ButtonUpdate(
...
void ClearScene::BackTitleBGimmick() {
	if (!isBackTitleGimmick) {
		    isEnd_BackTitleGimmick = false;
		    backTitle_t_ = 0.0f;
		    backTitleState_ = B_In;
	...
	//
		    ...
		    if (backTitleState_ == B_In) {
			    backTitlePos_.x = MyMath::EaseOutQuadF(
			        backTitle_t_, backTitle_in_S_pos_.x, backTitle_in_E_pos_.x);
			    backTitlePos_.y = MyMath::EaseOutQuadF(
			        backTitle_t_, backTitle_in_S_pos_.y, backTitle_in_E_pos_.y);
		    ...
		    //
			    ...
			    backTitlePos_.x = MyMath::EaseInQuadF(
			        backTitle_t_, backTitle_out_S_pos_.x, backTitle_out_E_pos_.x);
			    backTitlePos_.y = MyMath::EaseInQuadF(
			        backTitle_t_, backTitle_out_S_pos_.y, backTitle_out_E_pos_.y);
...
void ClearScene::ClearGimmick() { 
	...
	if (!isClearGimmick_) {
		    ...
		    if (clearGimmickTimer_ >= 0.0f) {
			    isLogoGimmick = true;
		    }
		    //
		    if (clearGimmickTimer_ >= 0.1f) {
			    isBackTitleGimmick = true;
		    }
		    
		    if (clearGimmickTimer_ >= 0.2f) {
			    isEndButtonGimmick = true;
		    }

		    if (clearGimmickTimer_ >= 1.0f) {
			    clearGimmickTimer_ = 1.0f;
			    isClearGimmick_ = true;
		    }
		    //
		    else if (clearGimmickTimer_ < 1.0f) {
			    clearGimmickTimer_ += 0.01f;
		    }
	}
	//
	else if (isClearGimmick_) {
	}
}

void ClearScene::EndButtonGimmick() {
	if (!isEndButtonGimmick) {
		    isEnd_endButtonGimmick = false;
		    endButton_t_ = 0.0f;
		    endButtonState_ = B_In;
	...
	//
		    ...
		    if (endButtonState_ == B_In) {
			    endButtonPos_.x = MyMath::EaseOutQuadF(
			        endButton_t_, endButton_in_S_pos_.x, endButton_in_E_pos_.x);
			    endButtonPos_.y = MyMath::EaseOutQuadF(
			        endButton_t_, endButton_in_S_pos_.y, endButton_in_E_pos_.y);
		    ...
		    //
			    ...
			    endButtonPos_.x = MyMath::EaseInQuadF(
			        endButton_t_, endButton_out_S_pos_.x, endButton_out_E_pos_.x);
			    endButtonPos_.y = MyMath::EaseInQuadF(
			        endButton_t_, endButton_out_S_pos_.y, endButton_out_E_pos_.y);
